...
const PayModal: React.FC<PayModalProps> = ({ isOpen, onClose, payment, onPay }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
      <div className="w-full max-w-sm glass-card p-8 rounded-[2.5rem] shadow-2xl animate-in zoom-in-95 duration-300">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-xl font-black text-secondary tracking-tight">Make Payment</h2>
          <button onClick={onClose} className="p-2 glass rounded-xl text-gray-400 dark:text-gray-500 hover:text-red-500 transition-colors">
            <X size={18} />
          </button>
        </div>

        <p className="text-xs text-gray-500 dark:text-gray-400 mb-8 font-medium">
          Paying for <span className="font-bold text-secondary">{payment.title}</span>.
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="relative group">
            <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 group-focus-within:text-emerald-500 transition-colors" size={24} />
            <input
              autoFocus
              type="number"
              step="0.01"
              className="w-full pl-12 pr-4 py-5 glass-card bg-gray-50 dark:bg-white/5 text-3xl font-black text-secondary rounded-2xl focus:ring-2 focus:ring-emerald-500/20 outline-none placeholder-gray-300 transition-all"
              placeholder="0.00"
              value={amount}
